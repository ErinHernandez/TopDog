...
const missingStarPlayers: StarPlayer[] = [
...
function generateProjections(player: StarPlayer): PlayerProjections {
  ...
  if (position === 'WR') {
    ...
    const rushYards: number = Math.round(fantasyPoints * 0.1);
...
playerPool.forEach((player) => {
  const starPlayer: StarPlayer | undefined = missingStarPlayers.find(
...
const updatedPlayerPoolString: string = JSON.stringify(playerPool, null, 2);
...
const updatedPool: PlayerData[] = eval(updatedPlayerPoolString);
const malik = updatedPool.find((p) => p.name === 'Malik Nabers');
if (malik?.clayProj && malik?.clayProjections?.receiving) {
  console.log(
    `✅ Malik Nabers: ${malik.clayProj} fantasy points, ${malik.clayProjections.receiving.yards} receiving yards`
  );
}
