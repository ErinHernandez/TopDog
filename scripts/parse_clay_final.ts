...
async function parseClayFinal(): Promise<ClayProjection[]> {
  ...
  try {
    ...
    const outputPath: string = 'clay_projections_2025_final.json';
...
function parsePlayerLine(line: string): ClayProjection | null {
  try {
    ...
    // Split by spaces
    const parts: string[] = data.split(/\s+/).filter((part: string) => part.trim());
