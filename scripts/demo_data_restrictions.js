...
const { dataAccessControl } = require('../lib/dataAccessControl.js');

function demonstrateDataRestrictions() {
  console.log('🔒 DATA ACCESS CONTROL DEMONSTRATION');
  console.log('=' .repeat(60));

  // Current period status
  const period = dataAccessControl.getCurrentPeriod();
  const message = dataAccessControl.getPeriodMessage();
  const status = dataAccessControl.getDataAvailabilityStatus();
  ...
  const testCases = [
    { type: 'personal', userId: 'user123', requesterId: 'user123', label: 'User\'s own draft data' },
    { type: 'personal', userId: 'user123', requesterId: 'user456', label: 'Another user\'s draft data' },
  ...
  testCases.forEach(testCase => {
    const validation = dataAccessControl.validateExportRequest(
      testCase.type, 
      testCase.userId, 
      testCase.requesterId
    );

    const status = validation.allowed ? '✅ ALLOWED' : '❌ RESTRICTED';
  ...
  const { available, restricted } = dataAccessControl.getAvailableDataTypes('user123');
  ...
  const isDraftActive = dataAccessControl.isDraftPeriodActive();
  const isSeasonActive = dataAccessControl.isSeasonActive();
...
module.exports = { demonstrateDataRestrictions };