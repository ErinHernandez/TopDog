...
export interface ExportModalProps {
  isOpen: boolean;
  onClose: () => void;
  exportType?: 'draft' | 'tournament' | 'player' | 'user';
...
export default function ExportModal({
  ...
  const checkDataAccess = async (): Promise<void> => {
    try {
      const response = await fetch(`/api/export/${exportType}/${exportId}?userId=${userId}&check=true`);
      if (response.status === 403) {
        const data = await response.json() as RestrictionInfo;
      ...
      logger.error('Error checking data access', error instanceof Error ? error : new Error(String(error)));
    }
  };
            ...
            <div className="mb-4 text-left">
              <label className="flex items-center text-gray-300">
                <input
                  type="checkbox"
                  checked={anonymize}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setAnonymize(e.target.checked)}
                  className="mr-2"
                />
