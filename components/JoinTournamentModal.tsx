...
// CONSTANTS
// ============================================================================
const DRAFT_TIMER_SECONDS_30S = 30;
const DRAFT_TIMER_SECONDS_8H = 28800; // 8 hours in seconds
const DRAFT_TOTAL_ROUNDS = 18;
const DEFAULT_MAX_PARTICIPANTS = 12;
const MIN_WAITING_ROOMS = 5;
...
export default function JoinTournamentModal({ open, onClose, tournamentType = 'topdog' }: JoinTournamentModalProps) {
  ...
  const buttonStyleActive = useMemo(() => ({
    backgroundColor: '#3c3c3c',
    color: '#c7c7c7',
    width: '55%'
  }), []);

  const buttonStyleInactive = useMemo(() => ({
  ...
  const ensureMinimumRooms = async () => {
    try {
      ...
      const roomsData = roomsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as Array<{ id: string; type: string; status: string; participants: unknown[] }>;
      const waitingRooms = roomsData.filter(room => 
        room.type === config.type &&
        room.status === 'waiting'
      );
      
      if (waitingRooms.length < MIN_WAITING_ROOMS) {
        const roomsToCreate = MIN_WAITING_ROOMS - waitingRooms.length;
        ...
        for (let i = 0; i < roomsToCreate; i++) {
          await addDoc(collection(db!, 'draftRooms'), {
            ...
            draftOrder: [],
                      settings: {
            timerSeconds: selectedDraftSpeed === '8h' ? DRAFT_TIMER_SECONDS_8H : DRAFT_TIMER_SECONDS_30S,
  ...
  const handleEnterTournament = async () => {
    ...
    try {
      ...
      if (currentBalance < config.entryFee) {
        ...
        notify(`Insufficient balance. You need $${config.entryFee} but only have $${currentBalance}. Please deposit more funds.`, 'error');
        setIsJoining(false);
        return;
      }
      ...
      const timerSeconds = selectedDraftSpeed === '8h' ? DRAFT_TIMER_SECONDS_8H : DRAFT_TIMER_SECONDS_30S;
      logger.debug('Finding available rooms', { selectedDraftSpeed, timerSeconds });
      const roomsSnapshot = await getDocs(query(collection(db!, 'draftRooms'), limit(50)));
      const roomsData = roomsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as Array<{ id: string; type: string; status: string; participants: unknown[]; maxParticipants?: number; settings?: { timerSeconds: number } }>;
      const availableRooms = roomsData.filter(room =>
        room.type === config.type &&
        room.status === 'waiting' &&
        (room.participants?.length || 0) < (room.maxParticipants || DEFAULT_MAX_PARTICIPANTS) &&
      ...
      notify(`Error joining draft room: ${error.message}. Please try again.`, 'error');
    } finally {
      setIsJoining(false);
    }
  };

  const handleAuthSuccess = (authenticatedUser: { uid: string }) => {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="relative bg-gray-900 rounded-3xl shadow-2xl pt-8 px-8 z-10 overflow-hidden" style={modalContainerStyle}>
            ...
            <div className="mb-6">
              <div className="flex justify-center space-x-4">
                <button
                  ...
                  className="px-8 py-2 rounded-lg font-bold transition-colors hover:bg-[#3c3c3c]"
                  style={selectedDraftSpeed === '30s' ? buttonStyleActive : buttonStyleInactive}
                >
                  30 seconds per pick
                </button>
                <button
                  ...
                  className="px-8 py-2 rounded-lg font-bold transition-colors hover:bg-[#3c3c3c]"
                  style={selectedDraftSpeed === '8h' ? buttonStyleActive : buttonStyleInactive}
                >
                  8 hours per pick
                </button>
              </div>
