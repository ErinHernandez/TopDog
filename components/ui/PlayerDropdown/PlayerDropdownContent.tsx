...
export interface PlayerDropdownContentProps {
  ...
  onQueue?: (e: React.MouseEvent<HTMLButtonElement>) => void;
...
export default function PlayerDropdownContent({
  ...
  customStyles = {},
  className = ''
}: PlayerDropdownContentProps): React.ReactElement {
  ...
  const calculateHeight = (): number => {
    ...
    if ((player as { stats?: Record<string, PlayerStats> }).stats) {
      const stats = (player as { stats?: Record<string, PlayerStats> }).stats;
      if (stats) {
        availableYears = Object.keys(stats).length;
      }
    }
  ...
  const getSortedStatsEntries = (stats?: Record<string, PlayerStats>): Array<[string, PlayerStats]> => {
    if (!stats) return [];
  ...
  const playerStats = (player as { stats?: Record<string, PlayerStats> }).stats;
  const dropdownStyle = DROPDOWN_STYLES.DROPDOWN_CONTENT.getStyle?.(player) || {};
        ...
        <div className="flex-1 px-3">
          <div className="text-xs text-gray-400 flex justify-center items-center gap-4">
            ...
            <div className="text-center" style={{ minWidth: '40px' }}>
              ...
              <div className="text-white font-medium" style={{ fontSize: '14px' }}>
                {parseFloat(String((player as { projectedPoints?: number | string }).projectedPoints || 0)).toFixed(1) || 'N/A'}
              </div>
            </div>
          </div>
        </div>
          ...
          {player.position === 'QB' ? (
            ...
            <div className="rounded text-xs overflow-x-auto hide-scrollbar" >
              <div style={{ minWidth: '900px' }}>
                ...
                {/* Stats Rows */}
                {getSortedStatsEntries(playerStats).map(([year, stats], index) => (
                  <div key={`${year}-${index}`} className={DROPDOWN_STYLES.STATS_ROW.className} style={DROPDOWN_STYLES.STATS_ROW.style}>
                    <div className="absolute text-left" style={{ left: '6px', width: '40px' }}>{year}</div>
