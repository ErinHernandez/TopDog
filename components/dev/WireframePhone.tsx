...
export const WireframePhone: React.FC<WireframePhoneProps> = ({
  screen,
  width = 240,
  height = 500,
}) => {
  ...
  // Screen content area
  const screenX = inset + 12;
  const screenY = inset + 44;
  const screenW = width - inset * 2 - 24;
  const screenH = height - inset * 2 - 72;
      ...
      case 'Sign In':
        return (
          <>
            ...
            <rect x={screenX} y={screenY + 177} width={10} height={10} rx={2} stroke={MCM.lineActive} strokeWidth={1} fill="none" />
            <line x1={screenX + 15} y1={screenY + 182} x2={screenX + 70} y2={screenY + 182} stroke={MCM.lineActive} strokeWidth={1} />
            {/* Forgot? link on right - horizontal line like Sign Up Link */}
            <line x1={screenX + 130} y1={screenY + 182} x2={screenX + screenW} y2={screenY + 182} stroke={MCM.lineActive} strokeWidth={1} />
  ...
  return (
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
      {/* Phone outline */}
      <rect x={inset} y={inset} width={width - inset * 2} height={height - inset * 2} rx={cornerRadius} stroke={MCM.lineActive} strokeWidth={2} fill="none" />

      {/* Corner nodes */}
      {[[inset, inset + cornerRadius], [inset + cornerRadius, inset], [width - inset - cornerRadius, inset], [width - inset, inset + cornerRadius], [width - inset, height - inset - cornerRadius], [width - inset - cornerRadius, height - inset], [inset + cornerRadius, height - inset], [inset, height - inset - cornerRadius]].map(([x, y], i) => (
        <circle key={i} cx={x} cy={y} r={4} fill={MCM.orange} />
      ))}

      {/* Dynamic Island */}
      <rect x={width/2 - 40} y={inset + 10} width={80} height={24} rx={12} fill="#000" />

      {/* Home indicator */}
      <rect x={width/2 - 45} y={height - inset - 16} width={90} height={4} rx={2} fill={MCM.lineActive} />
