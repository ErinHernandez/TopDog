...
export function useCardHeight(options: UseCardHeightOptions = {}): CardHeightResult {
  ...
  useEffect(() => {
    ...
    if (lockedDimensionsRef.current && !allowResize) {
      setIsReady(true);
      return;
    }
  ...
  // ----------------------------------------
  const locked = lockedDimensionsRef.current;

  return {
    height: locked?.height,
    width: locked?.width,
    availableHeight: locked ? locked.viewportHeight - locked.statusBarHeight - TAB_BAR_HEIGHT - topMargin - bottomMargin : undefined,
    availableWidth: locked ? locked.viewportWidth - leftMargin - rightMargin : undefined,
    statusBarHeight: locked?.statusBarHeight,
    tabBarHeight: TAB_BAR_HEIGHT,
    viewportHeight: locked?.viewportHeight,
    viewportWidth: locked?.viewportWidth,
    margins: {
      top: topMargin,
      bottom: bottomMargin,
      left: leftMargin,
    ...
    isReady,
    isLocked: !allowResize && locked !== null,
  };
}
