...
const STARTING_POSITIONS: RosterPosition[] = [
  'QB',
  'RB',
  'RB',
  'WR',
...
function getPlayerForSlot(
  ...
  const positionIndex =
    allPositions.slice(0, slotIndex + 1).filter(pos => pos === position).length - 1;
...
const PositionBadge = React.memo(function PositionBadge({
  position,
  size,
  ...
  return (
    <div className={cn(styles.badge, sizeClasses[size])} data-position={position.toLowerCase()}>
      {position}
    </div>
  );
});
...
const RosterRow = React.memo(function RosterRow({
  ...
  isExpanded = false,
  onToggleExpand,
}: RosterRowProps): React.ReactElement {
...
const RosterView = React.memo(function RosterView({
  ...
  const onTheClockIndex = isSnakeRound ? participantCount - 1 - pickIndexInRound : pickIndexInRound;
  ...
  return (
    <div className={styles.container}>
      ...
      <div ref={scrollContainerRef} onScroll={handleScroll} className={styles.rosterContainer}>
