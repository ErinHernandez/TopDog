...
export function useDraftPicks({
  ...
  const convertFirebasePick = useCallback((firebasePick: FirebasePickDocument, teamCount: number): DraftPick | null => {
    ...
    if (!participant) {
      logger.warn('Participant index out of bounds', { participantIndex, totalParticipants: participants.length });
      return null;
    }
  ...
  const picksByRoundMap = useMemo(() => {
    ...
    for (const pick of picks) {
      ...
      const roundPicks = map.get(round);
      if (roundPicks) {
  ...
  const picksByParticipantMap = useMemo(() => {
    ...
    for (const pick of picks) {
      ...
      const participantPicks = map.get(idx);
      if (participantPicks) {
        participantPicks.push(pick);
      }
    }
    return map;
  }, [picks]);
