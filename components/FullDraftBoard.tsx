...
interface PositionProportion {
  position: FantasyPosition;
  proportion: number;
  color: string;
}

export interface FullDraftBoardProps {
  room: DraftRoom | null;
  picks: DraftPick[];
  participants: string[];
  draftOrder: string[];
  PLAYER_POOL: PlayerInPool[];
}
...
const FullDraftBoard: React.FC<FullDraftBoardProps> = ({ 
  room, 
  picks, 
  participants, 
  draftOrder, 
  ...
  const [hasVisitedBoard, setHasVisitedBoard] = useState<boolean>(false);
  ...
  // Build picks grid
  const picksByTeam: string[][] = gridParticipants.map((team, teamIndex) =>
    rounds.map((round) => {
  ...
  const findPlayerInPool = (playerName: string): PlayerInPool | null => {
  ...
  const getTeamPositionProportions = (teamIdx: number): PositionProportion[] => {
  ...
  const handleScroll = (e: React.UIEvent<HTMLDivElement>): void => {
    if (hasVisitedBoard && e.target instanceof HTMLDivElement) {
            ...
            {rounds.map((round, rIdx) => (
              <tr key={round}>
                ...
                {gridParticipants.map((team, tIdx) => {
                  ...
                  let playerData: PlayerInPool | null = null;
