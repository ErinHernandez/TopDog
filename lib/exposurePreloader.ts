...
class ExposurePreloader {
  ...
  private processCSVData(csvText: string): ExposureData {
    ...
    picks.forEach(pick => {
      ...
      const pickNumber = parseInt(pick['Pick Number'] ?? '0') || 0;
      ...
      const entryFee = parseFloat(pick['Tournament Entry Fee'] ?? '0') || 0;
      ...
      if (!tournamentMap.has(tournament)) {
        tournamentMap.set(tournament, {
          ...
          entries: 0,
          userEntries: 0,
          entryFee: parseFloat(pick['Tournament Entry Fee'] ?? '0') || 0,
          prizes: parseFloat(pick['Tournament Total Prizes'] ?? '0') || 0,
          type: 'BB',
  ...
  init(): void {
    ...
    setTimeout(() => {
      this.startPreload().catch((err) => {
        logger.warn('Exposure preload failed during init', {
    ...
    const handleUserActivity = (): void => {
      ...
      activityTimeout = setTimeout(() => {
        if (!this.isPreloading && !userMetrics.getExposureDataCache()) {
          this.startPreload().catch((err) => {
            logger.warn('Exposure preload failed on user activity', {
              error: err instanceof Error ? err.message : String(err),
            });
          });
        }
