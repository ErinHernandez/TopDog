...
export async function logSecurityEvent(
  ...
  try {
    ...
    const sentryLevelMap = {
      [SecuritySeverity.CRITICAL]: 'fatal',
      [SecuritySeverity.HIGH]: 'error',
      [SecuritySeverity.MEDIUM]: 'warning',
      [SecuritySeverity.LOW]: 'info',
    };

    const sentryLevel = sentryLevelMap[severity] || 'info';

    // In production, send to Sentry for external logging
    if (process.env.NODE_ENV === 'production') {
      // Send to Sentry with appropriate context
      await captureMessage(`Security Event: ${eventType}`, {
        level: sentryLevel,
        tags: {
