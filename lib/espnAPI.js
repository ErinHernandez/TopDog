...
class ESPNPlayerAPI {
   ...
   */
  async fetchPlayerStats(playerId, season = 2024) {
    if (!playerId) return null;
        ...
        for (const category of data.stats.categories) {
          if (category.leaders) {
            ...
            if (playerLeader) {
              ...
              const value = parseFloat(playerLeader.value) || 0;
   ...
   */
  formatStatsForDisplay(playerData, playerPoolInfo) {
    if (!playerData) return null;

    const position = playerPoolInfo?.position || 'RB';
    const currentStats = playerData.currentStats || {};
    const previousStats = playerData.previousStats || {};

    return {
      name: playerPoolInfo?.name || playerData.info?.displayName || 'Unknown Player',
      position: position,
      team: playerPoolInfo?.team || 'UNKNOWN',
      seasons: [
        {
          year: 2024,
          games: currentStats.games || 17,
          passing: currentStats.passing || this.getDefaultStats('passing', position),
          rushing: currentStats.rushing || this.getDefaultStats('rushing', position),
          receiving: currentStats.receiving || this.getDefaultStats('receiving', position)
        },
        {
          year: 2023,
          games: previousStats.games || 16,
          passing: previousStats.passing || this.getDefaultStats('passing', position),
          rushing: previousStats.rushing || this.getDefaultStats('rushing', position),
          receiving: previousStats.receiving || this.getDefaultStats('receiving', position)
        }
      ],
  ...
  calculateCareerTotals(currentStats, previousStats, position) {
