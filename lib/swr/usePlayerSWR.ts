...
import { API_ENDPOINTS, CACHE_TIMES, fetcherLegacy } from './config';
...
/** Base SWR return type - moved here to avoid duplication */
...
export async function prefetchPlayerData(): Promise<void> {
  ...
  await Promise.allSettled(
    endpoints.map((url: string) => fetcherLegacy(url))
  );
}
