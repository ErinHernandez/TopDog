...
export type Platform = 'twitter' | 'facebook' | 'instagram' | 'linkedin' | 'whatsapp' | 'telegram' | 'reddit' | 'native' | 'download' | 'share';
...
export interface ShareDataInput {
  ...
  playerName?: string;
  playerId?: string;
  position?: string | number; // Can be player position (string) or leaderboard position (number)
  team?: string;
...
export const SHARE_TYPES: Record<string, ShareType> = {
...
const createShareData = (type: ShareType, data: ShareDataInput = {}): ShareData => {
...
export const generateShareData = (
  ...
  if (platform === 'twitter' || platform === 'instagram') {
    ...
    if (hashtags) {
      const maxTextWithHashtags = platformConfig.maxTextLength ? platformConfig.maxTextLength - hashtags.length - 1 : null;
      if (maxTextWithHashtags && shareData.text.length > maxTextWithHashtags) {
...
export const getPlatformShareUrl = (shareData: ShareData, platform: Platform): string => {
...
export const trackShare = (type: ShareType, platform: Platform, success: boolean = true): void => {
  if (typeof window !== 'undefined') {
    const windowWithGtag = window as unknown as { gtag?: (command: string, event: string, params: Record<string, unknown>) => void };
