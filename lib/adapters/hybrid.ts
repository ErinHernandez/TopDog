...
export class HybridAdapter<T extends DocumentData> {
    ...
    try {
      ...
      if (localData) {
        ...
        if (networkStatus.isOnline) {
          ...
          this.refreshFromServer(id).catch(() => {});
        }
  ...
  async getAll(): Promise<T[]> {
    ...
    if (localData.length > 0) {
      ...
      if (networkStatus.isOnline) {
        this.refreshAllFromServer().catch(() => {});
      }
  ...
  private async refreshAllFromServer(): Promise<void> {
    try {
      ...
      logger.debug('Background refresh all failed', {
        component: 'hybrid',
  ...
  async set(id: string, data: T): Promise<void> {
    ...
    if (networkStatus.isOnline) {
      this.syncSingle(id, 'set', data).catch(() => {
        ...
        logger.debug('Immediate sync failed, queued for later', {
          component: 'hybrid',
          operation: 'set',
          id,
        });
  ...
  async delete(id: string): Promise<void> {
    ...
    if (networkStatus.isOnline) {
      this.syncSingle(id, 'delete').catch(() => {
        logger.debug('Immediate delete sync failed, queued for later', {
          component: 'hybrid',
          operation: 'delete',
          id,
        });
