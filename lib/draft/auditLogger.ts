...
export class DraftAuditLogger {
  ...
  async init(): Promise<void> {
    ...
    this.flushInterval = setInterval(async () => {
      try {
        ...
        if (this.consecutiveFlushFailures >= 5) {
          logger.warn('Stopping flush interval due to repeated failures', {
            component: 'auditLogger'
          });
    ...
    cleanupRegistry.register('audit-logger', () => this.shutdown(), 'interval');
  ...
  async log(
    ...
    if (this.isDestroyed) {
      logger.warn('Attempted to log to destroyed audit logger', { component: 'auditLogger', action });
      return 'dropped_' + Date.now();
    }
  ...
  async shutdown(): Promise<void> {
    ...
    cleanupRegistry.unregister('audit-logger');
...
// WARNING: This singleton is created at module load time.
// Call auditLogger.init() before use and auditLogger.shutdown() on cleanup.
