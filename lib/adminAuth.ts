...
export async function verifyAdminAccess(
  ...
  if (!token) {
    return { isAdmin: false, error: 'Invalid authorization header format' };
  }
...
/**
 * Verify a Firebase ID token for general user authentication.
 * Unlike verifyAdminAccess, this does NOT check admin claims — it only
 * verifies the token is valid and the user exists.
 *
 * Use this for endpoints that require authentication but not admin access
 * (e.g., WebSocket connections, user-scoped API routes).
 */
export async function verifyUserToken(
  ...
  if (!firebaseAdminInitialized) {
    serverLogger.error(
      ...
      firebaseAdminInitError || new Error('Unknown initialization error')
    );
    return {
      authenticated: false,
