...
export class VIPAccountManager {
  ...
  static async getUsernameChangeHistory(uid: string): Promise<GetUsernameChangeHistoryResult> {
    try {
      ...
      history.sort((a, b) => {
        const aTime = a.changedAt instanceof Date ? a.changedAt.getTime() : (a.changedAt as Timestamp).toMillis();
        const bTime = b.changedAt instanceof Date ? b.changedAt.getTime() : (b.changedAt as Timestamp).toMillis();
        return bTime - aTime;
      });
...
module.exports = { VIPAccountManager, default: VIPAccountManager };
