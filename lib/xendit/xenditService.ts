 ...
 *
 * SECURITY: Verifies the integrity of the webhook payload using HMAC-SHA256.
 * This prevents request tampering attacks where an attacker modifies the
 * payload after it leaves Xendit's servers.
 *
 * The signature is passed in the x-xendit-signature header.
 *
 * @param rawBody - Raw request body as string
 * @param signature - Signature from x-xendit-signature header
 * @returns true if signature is valid, false otherwise
 */
export function verifyWebhookSignature(rawBody: string, signature: string | undefined): boolean {
  // SECURITY: Webhook secret is REQUIRED — reject all requests if not configured.
  // Without signature verification, attackers can send forged webhook payloads
  // to credit balances, trigger disbursements, or corrupt transaction state.
  if (!XENDIT_WEBHOOK_SECRET) {
    serverLogger.error('XENDIT_WEBHOOK_SECRET not configured - rejecting webhook', new Error('Missing webhook secret'), {
      component: 'xendit',
  ...
  if (!signature || typeof signature !== 'string') {
    serverLogger.warn('Missing webhook signature header', null, {
      component: 'xendit',
      operation: 'verifyWebhookSignature',
    });
    return false;
  ...
  try {
    ...
    if (signatureBuffer.length !== expectedBuffer.length) {
      serverLogger.warn('Webhook signature length mismatch', null, {
        component: 'xendit',
        operation: 'verifyWebhookSignature',
      });
      return false;
    ...
    if (!isValid) {
      serverLogger.warn('Webhook signature verification failed', null, {
        component: 'xendit',
        operation: 'verifyWebhookSignature',
      });
    }
    ...
    serverLogger.error('Error verifying webhook signature', error as Error, {
      component: 'xendit',
      operation: 'verifyWebhookSignature',
    });
    return false;
...
async function updateUserBalance(
  ...
  if (idempotencyKey) {
    ...
    if (!duplicateSnapshot.empty) {
      serverLogger.warn('Xendit: Duplicate balance operation detected', null, {
        userId,
        idempotencyKey,
        operation,
      });
  ...
  serverLogger.info('Xendit: Balance updated successfully', {
    userId,
    operation,
    amount,
