 ...
 */
async function savePdfToFile(pdfBuffer, filename = 'clay_projections.pdf') {
  try {
 ...
 */
async function createRealisticClayImage(pageNumber) {
  try {
 ...
 */
async function processRealClayPdf(pdfUrl, pageNumber = 1, analysisType = 'read') {
  try {
    ...
    try {
      ...
      console.warn('Could not clean up image file:', cleanupError.message);
    }
 ...
 */
async function processMultipleRealClayPdfPages(pdfUrl, startPage = 1, endPage = 5, analysisType = 'read') {
  const results = [];
 ...
 */
function findCompletePlayerEntries(lines) {
  const entries = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (rankMatch && parseInt(rankMatch[1]) <= 25) {
 ...
 */
function findMissedPlayerEntries(lines) {
  const missedEntries = [];
 ...
 */
function isCompletePlayerEntry(entry) {
 ...
 */
function updateEntryAnalysis(entry, line) {
 ...
 * Merge related entries for better completeness
 */
function mergeRelatedEntries(entries) {
  const mergedEntries = [];
  let i = 0;
 ...
 */
function parsePlayerWithValidation(entry) {
 ...
 */
function extractPlayerDataWithValidation(entry) {
  const playerData = {
    name: null,
    position: null,
    team: null,
 ...
 */
function extractPlayerInfoWithValidation(text, playerData) {
 ...
 */
function extractStatisticsWithValidation(text, playerData) {
 ...
 */
function extractNumbersWithValidation(text) {
  const numbers = [];
  ...
  if (numberMatches) {
    numberMatches.forEach((num, index) => {
      ...
      // Validate the number
      const value = num.includes('.') ? parseFloat(num) : parseInt(num.replace(/,/g, ''));
 ...
 */
function calculateNumberConfidence(num, beforeContext, afterContext) {
 ...
 */
function classifyNumbersWithAdvancedLogic(numbers, playerData) {
 ...
 */
function findFantasyPointsWithValidation(text, playerData) {
