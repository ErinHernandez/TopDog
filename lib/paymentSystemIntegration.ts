...
import { getPaymentMethodsByLocation, getOrderedPaymentMethods, type PaymentMethodId } from './paymentMethodConfig';
...
class PaymentSystemOrchestrator {
  ...
  private async selectFallbackProcessor(paymentRequest: PaymentRequest, failedProcessor: string): Promise<string | null> {
  ...
  private async executePayment(processor: string, paymentRequest: PaymentRequest, transactionId: string): Promise<PaymentResult> {
  ...
  // Webhook handling
  async handleWebhook(processor: string, payload: string, signature: string, secret: string): Promise<WebhookResult> {
    try {
