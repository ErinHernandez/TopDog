...
export const getPositionEndColor = (position) => {
  return '#1f2833';
};

// Create first gradient (0px to 136px) - 80% of the transition (15% faster)
export const createFirstGradient = (startColor, endColor) => {
  const gradientStops = [];
...
export const createSecondGradient = (startColor, endColor) => {
  const gradientStops = [];
...
export const createPositionGradient = (position) => {
...
export const createRosterGradient = (position) => {
  const positionColor = getPositionColor(position);
  const positionEndColor = getPositionEndColor(position);
...
export const createQueueGradient = (position) => {
  const positionColor = getPositionColor(position);
  const positionEndColor = getPositionEndColor(position);
...
export const createPickedPlayerGradient = (position) => {
  const positionColor = getPositionColor(position);
  const positionEndColor = getPositionEndColor(position);
  ...
  if (position === 'RB') {
    ...
    const remainingDistance = 37.4765625; // 40% of the full 93.69140625px distance
    ...
    // Complete the remaining 40% of the transition
    const remainingDistance = 41.484375; // 40% of the full 103.7109375px distance
    ...
    // Complete the remaining 40% of the transition
    const remainingDistance = 38.25; // 40% of the full 95.625px distance
...
export const getTeamColors = (team) => {
  const teamColors = {
    ...
    'JAC': ['#101820', '#D7A22A'], // Jaguars (alt code): Black, Gold
    ...
    'TB': ['#D50A0A', '#000000'], // Buccaneers: Red, Black
...
export const createTeamGradient = (team) => {
  ...
  // Create corner-to-corner gradient (top-right to bottom-left) with lighter color on edges, darker in center
  const diagonalGradient = `linear-gradient(225deg, ${edgeColor} 0%, ${centerColor} 50%, ${edgeColor} 100%)`;
...
export const createFlexGradient = () => {
