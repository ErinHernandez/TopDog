...
/**
 * Check if we're running on Vercel (client-side detection)
 * @returns {boolean}
 */
const isVercelDeployment = () => {
  ...
  // Client-side: Check hostname for Vercel domains
  if (typeof window !== 'undefined' && window.location) {
    const hostname = window.location.hostname;
    // Vercel preview/production deployments use *.vercel.app
...
export const canAccessDevFeatures = (userId = null, accessToken = null, authToken = null) => {
  ...
  // Check if barrier is explicitly enabled (overrides Vercel detection)
