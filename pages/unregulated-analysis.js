...
export default function UnregulatedAnalysis() {
  const { user } = useUser();
  ...
  const getStateData = (abbreviation) => {
  ...
  const sortedStates = [...allStates].sort((a, b) => {
    ...
    switch(sortBy) {
      ...
      case 'regNumber':
        ...
        const aBaseNum = parseFloat(aBase) || 0;
        const bBaseNum = parseFloat(bBase) || 0;
      ...
      case 'applicationCost':
        aValue = removeParentheses(a.applicationCost || (a.regNumber === 0 ? '$0' : a.regNumber === 50 ? 'TBD' : '$0'));
        bValue = removeParentheses(b.applicationCost || (b.regNumber === 0 ? '$0' : b.regNumber === 50 ? 'TBD' : '$0'));
        break;
      case 'licenseCost':
        aValue = removeParentheses(a.licenseCost || (a.regNumber === 0 ? '$0' : a.regNumber === 50 ? 'TBD' : '$0'));
        bValue = removeParentheses(b.licenseCost || (b.regNumber === 0 ? '$0' : b.regNumber === 50 ? 'TBD' : '$0'));
        break;
      case 'renewalCost':
        aValue = removeParentheses(a.renewalCost || (a.regNumber === 0 ? '$0' : a.regNumber === 50 ? 'TBD' : '$0'));
        bValue = removeParentheses(b.renewalCost || (b.regNumber === 0 ? '$0' : b.regNumber === 50 ? 'TBD' : '$0'));
        break;
      case 'taxRate':
        aValue = removeParentheses(a.taxRate === 'None' ? '0%' : a.taxRate || (a.regNumber === 0 ? '0%' : a.regNumber === 50 ? '0%' : '0%'));
        bValue = removeParentheses(b.taxRate === 'None' ? '0%' : b.taxRate || (b.regNumber === 0 ? '0%' : b.regNumber === 50 ? '0%' : '0%'));
        break;
      default:
        aValue = a.state;
        bValue = b.state;
          ...
          <div className="relative">
            ...
            >
              {Object.entries(usStatesMap).map(([abbreviation, stateData]) => {
                const stateInfo = getStateData(abbreviation);
                ...
                return (
                  <g key={abbreviation}>
                    <path
                      ...
                      onClick={() => setSelectedState(selectedState === abbreviation ? null : abbreviation)}
                    />
                    {isHovered && (
                      <text
                        x="400"
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {sortedStates.map((state, index) => (
                  <tr key={state.abbreviation} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center" style={{minWidth: '20px'}}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center" style={{minWidth: '160px'}}>
                      <div className="max-w-xs">
                        {state.applicationCost || (state.regNumber === 0 ? '$0' : state.regNumber === 50 ? 'TBD' : '$0')}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center" style={{minWidth: '160px'}}>
                      <div className="max-w-xs">
                        {state.licenseCost || (state.regNumber === 0 ? '$0' : state.regNumber === 50 ? 'TBD' : '$0')}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                      <div className="max-w-xs">
                        {state.renewalCost || (state.regNumber === 0 ? '$0' : state.regNumber === 50 ? 'TBD' : '$0')}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                      <div className="max-w-xs">
                        {state.taxRate === 'None' ? '0%' : state.taxRate || (state.regNumber === 0 ? '0%' : state.regNumber === 50 ? '0%' : '0%')}
                      </div>
                    </td>
