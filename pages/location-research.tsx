...
interface OfficialDocuments {
  bills?: OfficialDocument[];
  codes?: OfficialDocument[];
  regulations?: string[];
  courtDecisions?: Array<{ id: string; title: string; date: string; court: string; caseNumber: string; summary: string; fantasySportsImpact: string; status: string }> | OfficialDocument[] | string[];
...
interface PendingLegislation {
  ...
  status?: string;
  description?: string;
  bill?: string; // Alternative property name used in some locations
  committee?: string; // Committee information
...
interface SportsData {
  popularity: {
    americanFootball: { popularity: number; viewership: number; participation: number; fantasyParticipation: number };
    basketball: { popularity: number; viewership: number; participation: number; fantasyParticipation: number };
    baseball: { popularity: number; viewership: number; participation: number; fantasyParticipation: number };
    soccer: { popularity: number; viewership: number; participation: number; fantasyParticipation: number };
    hockey: { popularity: number; viewership: number; participation: number; fantasyParticipation: number };
  };
  leagues: Record<string, any>; // Complex nested structure - using Record for flexibility
  fantasySports?: Record<string, any>; // Fantasy sports data within sports object
  gambling?: Record<string, any>; // Gambling data within sports object
}
...
interface LocationData {
  ...
  sports?: SportsData; // Add optional sports property
  demographics?: Record<string, any>; // Demographics data
  economy?: Record<string, any>; // Economy data
  technology?: Record<string, any>; // Technology data
  business?: Record<string, any>; // Business data
  political?: Record<string, any>; // Political data
}

type SortBy = 'name' | 'state' | 'status';
type SortOrder = 'asc' | 'desc';
type ViewMode = 'list' | 'grid';

// Nevada - Season-long fantasy sports not allowed
...
export default function LocationResearch(): JSX.Element {
  const { user } = useUser();
  const [locations, setLocations] = useState<LocationData[]>([initialAlaskaData, initialArizonaData, initialCaliforniaData, initialColoradoData, initialConnecticutData, initialDelawareData, initialFloridaData, initialGeorgiaData, initialIdahoData, initialIllinoisData, initialIndianaData, initialIowaData, initialKentuckyData, initialLouisianaData, initialMassachusettsData, initialMichiganData, initialMinnesotaData, initialMississippiData, initialMontanaData, initialNebraskaData, initialNevadaData, initialNewJerseyData, initialNewMexicoData, initialNewYorkData, initialNorthCarolinaData, initialNorthDakotaData, initialOklahomaData, initialOregonData, initialPennsylvaniaData, initialRhodeIslandData, initialSouthCarolinaData, initialSouthDakotaData, initialTennesseeData, initialTexasData, initialUtahData, initialWashingtonData, initialWisconsinData]);
  const [selectedLocation, setSelectedLocation] = useState<LocationData | null>(initialNevadaData);
  const [showAddModal, setShowAddModal] = useState<boolean>(false);
  const [filteredLocations, setFilteredLocations] = useState<LocationData[]>([initialAlaskaData, initialArizonaData, initialCaliforniaData, initialColoradoData, initialConnecticutData, initialDelawareData, initialFloridaData, initialGeorgiaData, initialIdahoData, initialIllinoisData, initialIndianaData, initialIowaData, initialKentuckyData, initialLouisianaData, initialMassachusettsData, initialMichiganData, initialMinnesotaData, initialMississippiData, initialMontanaData, initialNebraskaData, initialNevadaData, initialNewJerseyData, initialNewMexicoData, initialNewYorkData, initialNorthCarolinaData, initialNorthDakotaData, initialOklahomaData, initialOregonData, initialPennsylvaniaData, initialRhodeIslandData, initialSouthCarolinaData, initialSouthDakotaData, initialTennesseeData, initialTexasData, initialUtahData, initialWashingtonData, initialWisconsinData]);
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600">Research and analyze locations for fantasy sports compliance</p>
        </div>
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <span className={`px-2 py-1 text-xs rounded-full ${
                        location.status === 'research' ? 'bg-yellow-100 text-yellow-800' :
            ...
            {selectedLocation ? (
              <div className="bg-white rounded-lg shadow">
                <div className="p-6 border-b border-gray-200">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-gray-600">{selectedLocation.country} • {selectedLocation.state}</p>
                    </div>
                    <span className={`px-3 py-1 text-sm rounded-full ${
                      selectedLocation.status === 'research' ? 'bg-yellow-100 text-yellow-800' :
                      selectedLocation.status === 'approved' ? 'bg-green-100 text-green-800' :
                      selectedLocation.status === 'denied' ? 'bg-red-100 text-red-800' :
                      'bg-gray-100 text-gray-800'
                    }`}>
                ...
                <div className="p-6">
                  ...
                  {/* Compliance Section */}
                  {selectedLocation.compliance && (
                    <div className="space-y-6">
                      <h3 className="text-lg font-semibold text-gray-900">Compliance</h3>
                      
                      {/* Gambling */}
                      {selectedLocation.compliance.gambling && (
                        <div className="bg-gray-50 rounded-lg p-4">
                          <h4 className="font-medium text-gray-900 mb-3">Gambling</h4>
                          
                          {/* General Gambling */}
                          {selectedLocation.compliance.gambling.generalGambling && (
                            <div className="mb-4">
                              <h5 className="text-sm font-medium text-gray-700 mb-2">General Gambling</h5>
                              <div className="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                  <span className="text-gray-600">Legal:</span>
                                  <span className={`ml-2 font-medium ${selectedLocation.compliance.gambling.generalGambling.legal ? 'text-green-600' : 'text-red-600'}`}>
                                    {selectedLocation.compliance.gambling.generalGambling.legal ? 'Yes' : 'No'}
                                  </span>
                                </div>
                                <div>
                                  <span className="text-gray-600">Sports Betting:</span>
                                  <span className={`ml-2 font-medium ${selectedLocation.compliance.gambling.generalGambling.sportsBetting ? 'text-green-600' : 'text-red-600'}`}>
                                    {selectedLocation.compliance.gambling.generalGambling.sportsBetting ? 'Yes' : 'No'}
                                  </span>
                                </div>
                              </div>
                              {selectedLocation.compliance.gambling.generalGambling.notes && (
                                <p className="text-sm text-gray-600 mt-2">{selectedLocation.compliance.gambling.generalGambling.notes}</p>
                              )}
                            </div>
                          )}

                          {/* Fantasy Sports */}
                          {selectedLocation.compliance.gambling.fantasySports && (
                            <div>
                              <h5 className="text-sm font-medium text-gray-700 mb-2">Fantasy Sports</h5>
                              
                              {/* Season Long */}
                              {selectedLocation.compliance.gambling.fantasySports.seasonLong && (
                                <div className="mb-3">
                                  <h6 className="text-xs font-medium text-gray-600 mb-1">Season Long</h6>
                                  <div className="grid grid-cols-2 gap-4 text-sm">
                                    <div>
                                      <span className="text-gray-600">Legal:</span>
                                      <span className={`ml-2 font-medium ${selectedLocation.compliance.gambling.fantasySports.seasonLong.legal ? 'text-green-600' : 'text-red-600'}`}>
                                        {selectedLocation.compliance.gambling.fantasySports.seasonLong.legal ? 'Yes' : 'No'}
                                      </span>
                                    </div>
                                    <div>
                                      <span className="text-gray-600">Classification:</span>
                                      <span className="ml-2 font-medium text-gray-900">
                                        {selectedLocation.compliance.gambling.fantasySports.seasonLong.classification}
                                      </span>
                                    </div>
                                  </div>
                                  {selectedLocation.compliance.gambling.fantasySports.seasonLong.notes && (
                                    <p className="text-sm text-gray-600 mt-1">{selectedLocation.compliance.gambling.fantasySports.seasonLong.notes}</p>
                                  )}
                                </div>
                              )}
                            </div>
