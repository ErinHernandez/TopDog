/**
 * Deposit Page - TopDog.dog
 *
 * Main deposit/funding page with multi-method payment support.
 *
 * COMPONENT EXTRACTION ROADMAP (Future Refactoring):
 * ========================================
 * Current Size: 1,314 lines, 23 useState calls
 *
 * This component is planned to be split into sub-components:
 * 1. DepositForm - Main form wrapper and state management (handles user location, compliance)
 * 2. PaymentMethodSelector - Payment method selection UI (PARTIALLY EXTRACTED - see /components/payments/)
 * 3. StripePayment - Stripe-specific payment interface and logic
 * 4. PayPalPayment - PayPal-specific payment interface and logic
 * 5. Additional specialized payment handlers (Apple Pay, Google Pay, etc.)
 *
 * Priority: Extract Stripe and PayPal into separate hooks/components to reduce main component size
 * Note: PCI compliance changes already implemented - preserve CardElement handling
 */
...
export default function DepositPage() {
  ...
  const getUserLocation = async (): Promise<void> => {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      if (methods.length > 0) {
        setSelectedMethod(methods[0]!.toString());
      }
      setLoading(false);
      return;
    }
  ...
  const checkCompliance = async (): Promise<void> => {
    try {
      // Note: canMakeDeposits expects (userData, country) but page calls with (userId)
      // Using type assertion to bridge the gap
      const userData = user ? { id: user.uid } : null;
              ...
              <div className="grid grid-cols-2 gap-8" style={{ minWidth: '1400px' }}>
                ...
                {selectedMethod && (
                  <div className="lg:col-span-1">
                    ...
                    ) : selectedMethod === 'apple_pay' || selectedMethod === 'applepay' ? (
                    ...
                    ) : selectedMethod === 'google_pay' || selectedMethod === 'googlepay' ? (
