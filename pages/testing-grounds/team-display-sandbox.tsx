...
interface PlayerExpandedCardNoDraftProps {
  player: {
    name: string;
    team: string;
    position: 'QB' | 'RB' | 'WR' | 'TE';
    adp: number | string | null | undefined;
    projectedPoints: number | string | null | undefined;
  };
  onClose?: () => void;
  hideOuterGradient?: boolean;
}
...
function formatStatsForPosition(position: 'QB' | 'RB' | 'WR' | 'TE', stats: SeasonStats | null): string[] {
...
function PlayerExpandedCardNoDraft({ player, onClose, hideOuterGradient = false }: PlayerExpandedCardNoDraftProps): JSX.Element | null {
  ...
  const byeWeek = (team && team in BYE_WEEKS) ? BYE_WEEKS[team as keyof typeof BYE_WEEKS] ?? 'N/A' : 'N/A';
      ...
      <div style={{
        ...
        <div style={{ flex: 1, display: 'flex', justifyContent: 'space-around', alignItems: 'center' }}>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 600, color: '#fff' }}>{parseFloat(String(adp ?? 0)).toFixed(1)}</div>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 10, color: '#9ca3af', letterSpacing: '0.5px' }}>PROJ</div>
            <div style={{ fontSize: 15, fontWeight: 600, color: '#fff' }}>{parseFloat(String(projectedPoints ?? 0)).toFixed(1)}</div>
          </div>
        </div>
      </div>
...
// Position colors
const POSITION_COLORS: Record<'QB' | 'RB' | 'WR' | 'TE', string> = {
  QB: '#F472B6',
  RB: '#0fba80', 
  WR: '#FBBF25',
  TE: '#7C3AED',
...
function groupByPosition(players: Player[]): Array<{ position: 'QB' | 'RB' | 'WR' | 'TE'; players: Player[] }> {
  const groups: Record<string, Player[]> = {};
  const posOrder: Array<'QB' | 'RB' | 'WR' | 'TE'> = ['QB', 'RB', 'WR', 'TE'];
...
function PositionBadge({ position }: { position: 'QB' | 'RB' | 'WR' | 'TE' }): JSX.Element {
...
// ============================================================================
function DesignC({ team }: DesignCProps): JSX.Element {
  const [expandedPlayer, setExpandedPlayer] = useState<string | null>(null);
  const expandedRef = useRef<HTMLDivElement>(null);
  const grouped = groupByPosition(team.players);
  
  const togglePlayer = (playerName: string): void => {
...
// MAIN SANDBOX PAGE
// ============================================================================
function TeamDisplaySandbox(): JSX.Element {
