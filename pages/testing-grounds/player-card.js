...
function PlayerCardTest() {
  ...
  const [overallPick, setOverallPick] = useState(6); // Overall pick number (1, 2, 3, 4, 5, 6, etc.) - set to 6 for pick 1.06
  const [picksMade, setPicksMade] = useState([]); // Track all picks made - start with no picks
  ...
  const [isDraftActive, setIsDraftActive] = useState(true); // Set to true so timer runs
  const [draftTimer, setDraftTimer] = useState(30);
  const [currentDraftingTeam, setCurrentDraftingTeam] = useState(1);
  const [availablePlayers, setAvailablePlayers] = useState([]);
  const [isDraftPaused, setIsDraftPaused] = useState(false);
  const [showOverallPick, setShowOverallPick] = useState(false);
  ...
  const [isHoveringButtons, setIsHoveringButtons] = useState(false); // Track if hovering over button area
  const [hoverButtonTimer, setHoverButtonTimer] = useState(null); // Timer for hover delay
  const [pickNumberFormat, setPickNumberFormat] = useState('round.pick'); // Track pick number format: 'round.pick' or 'overall'
  ...
  const getTeamForPick = (pickIndex) => {
  ...
  const simulatePicks = () => {
    ...
    for (let i = 0; i < 206; i++) {
      ...
      const player = players[i] || { name: `Player ${i + 1}`, position: 'RB', team: 'FA' };
          ...
          <div className="relative" style={{ position: 'relative' }}>
            ...
            {Array.from({ length: 1 }, (_, setIndex) => 
              Array.from({ length: 216 }, (_, i) => {
                ...
                // Define the players for all 18 rounds (216 picks total)
                const players = [
                  // Round 1 picks (1-12)
                  { name: 'Saquon Barkley', position: 'RB', team: 'NYG' },
                  { name: 'Ja\'Marr Chase', position: 'WR', team: 'CIN' },
                    ...
                    <div className="text-xs text-center flex justify-center" style={{ position: 'absolute', bottom: '11px', left: '50%', transform: 'translateX(-50%)', zIndex: 9999, width: '90%' }}>
                      <div style={{ height: 16, width: '100%', borderRadius: 6, display: 'flex', overflow: 'hidden', backgroundColor: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)' }}>
                        {(() => {
                          ...
                          const getPositionPercentagesForCard = (cardIndex) => {
