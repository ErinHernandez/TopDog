...
type PresetKey = 'homepage' | 'socialSquare' | 'socialWide' | 'thumbnail' | 'banner' | 'custom';
...
const PRESETS: Record<PresetKey, BoardConfig> = {
  homepage: {
    name: 'Homepage Hero',
    columns: 12,
    rows: 18,
...
function Slider({ label, value, onChange, min, max, step = 1 }: SliderProps): JSX.Element {
  return (
...
function NumberInput({ label, value, onChange, min, max }: NumberInputProps): JSX.Element {
  return (
    <div className="space-y-1">
...
function MarketingBoardPage(): JSX.Element {
  const boardRef = useRef<HTMLDivElement>(null);
  const [activePreset, setActivePreset] = useState<PresetKey>('homepage');
  const [animated, setAnimated] = useState<boolean>(false);
  const [animationKey, setAnimationKey] = useState<number>(0);
  
  // Board configuration state
  const [config, setConfig] = useState<BoardConfig>(PRESETS.homepage);
  
  // Handle preset change
  const handlePresetChange = useCallback((presetKey: PresetKey): void => {
  ...
  const updateConfig = useCallback((key: keyof BoardConfig, value: number | string): void => {
    setConfig((prev) => ({ ...prev, [key]: value }));
    setActivePreset('custom');
  }, []);
  
  // Replay animation
  const replayAnimation = useCallback((): void => {
  ...
  const exportAsImage = useCallback(async (): Promise<void> => {
