...
export default function InitDevTournaments() {
  ...
  const handleAddSpecific = async (tournamentKey) => {
    setIsInitializing(true);
    setMessage(`Adding ${devTournamentTemplates[tournamentKey].name}...`);
    
    try {
      await addTournamentToDevelopment(tournamentKey);
      setMessage(`${devTournamentTemplates[tournamentKey].name} added successfully!`);
    } catch (error) {
      setMessage(`Error: ${error.message}`);
    } finally {
      setIsInitializing(false);
