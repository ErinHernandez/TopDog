...
const logger = createScopedLogger('[Leaderboard]')
...
export default function Leaderboard() {
  ...
  const { data: leaderboardData = [], isLoading, error } = useSWR<LeaderboardEntry[]>(
    '/api/leaderboard',
    fetcher,
    {
  ...
  // Sort data by points (descending) and rank
  const sortedData = useMemo(() => {
    const sorted = [...leaderboardData].sort((a, b) => {
      ...
      // Secondary sort: by record (wins - losses) descending
  ...
  const filteredData = selectedLeague === 'all'
    ? sortedData
    : sortedData.filter(entry => entry.league === selectedLeague)

  const leagues = [...new Set(sortedData.map(entry => entry.league))]
      ...
      <div className="min-w-[1400px] zoom-stable" style={{ minWidth: '1400px', transform: 'translateZ(0)' }}>
        ...
        <main className="container py-[18px] zoom-resistant" style={{ minWidth: '1400px', transform: 'translateZ(0)' }}>
          <div className="mb-8">
            ...
            {isLoading && <p className="text-sm text-gray-500 mt-2">Loading leaderboard data...</p>}
            {error && <p className="text-sm text-red-500 mt-2">Failed to load leaderboard. Please try again.</p>}
          </div>
