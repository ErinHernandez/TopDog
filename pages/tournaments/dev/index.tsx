...
export default function DevTournaments() {
  ...
  const fetchDevTournaments = async () => {
    ...
    try {
      ...
      logger.error('Error fetching dev tournaments:', error instanceof Error ? error : new Error(String(error)));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateTournament = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      logger.error('Error creating tournament:', error instanceof Error ? error : new Error(String(error)));
    }
  };
  ...
  const handleDeleteTournament = async (tournamentId: string) => {
    ...
    if (confirm('Are you sure you want to delete this tournament?')) {
      try {
        ...
        logger.error('Error deleting tournament:', error instanceof Error ? error : new Error(String(error)));
      }
    }
  };
  ...
  const handleStatusChange = async (tournamentId: string, newStatus: string) => {
    ...
    try {
      ...
      logger.error('Error updating tournament status:', error instanceof Error ? error : new Error(String(error)));
    }
  };
