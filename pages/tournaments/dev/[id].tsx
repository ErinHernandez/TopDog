...
export default function DevTournamentDetail() {
  ...
  const fetchTournament = async () => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        const tournamentData: Tournament = { id: docSnap.id, ...data } as Tournament;
        setTournament(tournamentData);
        setEditForm(tournamentData);
      } else {
  ...
  const createFromTemplate = async (templateKey: string) => {
    const template = devTournamentTemplates[templateKey as keyof typeof devTournamentTemplates];
    if (template) {
      setEditForm({
        ...template,
              ...
              <div>
                ...
                <div className="flex items-center space-x-4">
                  <span 
                    ...
                    style={{ 
                      backgroundColor: (tournamentStatuses.find(s => s.value === tournament.status)?.color || '#59c5bf') + '20',
                      color: tournamentStatuses.find(s => s.value === tournament.status)?.color || '#59c5bf'
                    }}
                  >
                    {tournamentStatuses.find(s => s.value === tournament.status)?.label || tournament.status}
                  </span>
                ...
                <div className="grid md:grid-cols-2 gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setEditForm({...editForm, maxEntries: parseInt(e.target.value) || 0})}
                      className="w-full p-3 rounded-lg bg-gray-700 text-white border border-gray-600"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setEditForm({...editForm, prizePool: parseInt(e.target.value) || 0})}
                      className="w-full p-3 rounded-lg bg-gray-700 text-white border border-gray-600"
                    />
                  </div>
                  <div>
              ...
              <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="text-white font-semibold">${(tournament.entryFee || 0).toLocaleString()}</div>
                  </div>
                  <div>
                    <span className="text-gray-400 text-sm">Max Entries</span>
                    <div className="text-white font-semibold">{(tournament.maxEntries || 0).toLocaleString()}</div>
                  </div>
                  <div>
                    <span className="text-gray-400 text-sm">Prize Pool</span>
                    <div className="text-white font-semibold">${(tournament.prizePool || 0).toLocaleString()}</div>
                  </div>
                  <div>
                    <span className="text-gray-400 text-sm">Format</span>
                    <div className="text-white font-semibold">{tournament.format || 'N/A'}</div>
                  </div>
                  <div>
                    <span className="text-gray-400 text-sm">League Size</span>
                    <div className="text-white font-semibold">{tournament.leagueSize || 0} Teams</div>
                  </div>
                </div>
              </div>
