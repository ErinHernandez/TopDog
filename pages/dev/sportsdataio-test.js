...
export default function SportsDataIOTest() {
  ...
  const runTest = async (endpoint) => {
    ...
    try {
      ...
      setResults(prev => ({ ...prev, [endpoint.name]: { error: err.message } }));
    } finally {
      setLoading(null);
    }
  };
  ...
  const clearCache = async () => {
    ...
    try {
      ...
      await runTest(ENDPOINTS.find(e => e.name === 'Cache Status'));
    } catch (err) {
                ...
                {result && isExpanded && (
                  <div style={{ 
                    ...
                    {endpoint.name === 'Projections' && result.data && (
                      <div style={{ marginBottom: '16px' }}>
                        ...
                        <div style={{ display: 'grid', gap: '4px' }}>
                          {result.data.slice(0, 10).map((p, i) => (
                            <div key={i} style={{ 
                              display: 'flex', 
                              justifyContent: 'space-between',
                              fontSize: '13px',
                              ...
                              <span>
                                ...
                                <span style={{ 
                                  color: p.Position === 'QB' ? '#F472B6' : 
                                         p.Position === 'RB' ? '#0fba80' : 
                                         p.Position === 'WR' ? '#4285F4' : '#7C3AED',
                                  marginLeft: '8px',
                                  fontSize: '11px'
                                }}>
                                  {p.Position}
                              ...
                              <span style={{ color: '#22c55e' }}>
                                {(p.FantasyPointsPPR || 0).toFixed(1)} pts
                              </span>
                            </div>
                          ))}
                        </div>
                    ...
                    {endpoint.name === 'Fantasy ADP' && result.data && (
                      <div style={{ marginBottom: '16px' }}>
                        <h4 style={{ color: '#94a3b8', fontSize: '12px', marginBottom: '8px' }}>
                          ADP RANKINGS ({result.scoringType?.toUpperCase() || 'PPR'})
                        </h4>
                        <div style={{ display: 'grid', gap: '4px' }}>
                          {result.data.slice(0, 12).map((p, i) => (
                            <div key={i} style={{ 
                              display: 'flex', 
                              justifyContent: 'space-between',
                              fontSize: '13px',
                            ...
                            <div key={i} style={{ 
                              ...
                              <span>
                                <span style={{ color: '#fbbf24', marginRight: '12px', fontSize: '11px' }}>
                                  ADP: {(p.adpPPR || p.adp || 0).toFixed(1)}
                                </span>
                                <span style={{ color: '#22c55e' }}>
                                  {(p.projectedPointsPPR || p.projectedPoints || 0).toFixed(0)} pts
                                </span>
                              </span>
                            </div>
                          ))}
                    ...
                    {endpoint.name === 'Teams' && result.data && (
                      <div style={{ marginBottom: '16px' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px' }}>
                          {result.data.slice(0, 16).map((t, i) => (
                            <div key={i} style={{ 
                              display: 'flex', 
                              alignItems: 'center',
                              gap: '8px',
                        ...
                        {result.data[0] && (
                          <div style={{ marginTop: '12px', padding: '12px', backgroundColor: '#1e293b', borderRadius: '6px' }}>
                            <div style={{ fontSize: '11px', color: '#64748b', marginBottom: '4px' }}>
                              Sample: {result.data[0].fullName}
                            </div>
                            <div style={{ fontSize: '12px', color: '#94a3b8' }}>
                              Coach: {result.data[0].headCoach} | 
                              Stadium: {result.data[0].stadium?.name} | 
                              Bye: Week {result.data[0].byeWeek}
                            </div>
                          </div>
                        )}
                      </div>
                          ...
                          {Object.entries(result.caches).map(([key, cache]) => (
                            <div key={key} style={{ 
                              ...
                              <span style={{ 
                                ...
                                {cache.exists 
                                  ? cache.isValid 
                                    ? `Valid (${cache.ageMinutes}m old, ${cache.itemCount} items)`
