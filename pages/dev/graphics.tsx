...
export default function DevGraphics() {
  ...
  const ColorSwatch = ({ color, label, size = 'normal' }: ColorSwatchProps) => {
    if (!color) return (
      <div style={{ 
        display: 'flex', 
        alignItems: 'center', 
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          {Object.keys(grouped).sort().map(division => (
            <button
              key={division}
              onClick={() => {
                const element = document.getElementById(division.replace(' ', '-'));
            ...
            <div 
              ...
              onClick={() => {
                if (team.colors?.primary) {
                  copyToClipboard(team.colors.primary, `${team.key} primary`);
                }
              }}
            >
              <div style={{
