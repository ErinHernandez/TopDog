...
import { reserveUsernameSchema } from '../../../../lib/validation/auth';
...
const handler = async function(
  ...
  return withErrorHandling(req, res, async (req, res, logger): Promise<unknown> => {
    ...
    const parsed = validateRequestBody(req, reserveUsernameSchema, logger);
    const {
      username,
      reservedFor,
    ...
    // Create reservation ID and expiry upfront (needed for transaction)
    ...
    // Use transaction for atomic check-and-set of username reservation
    // This prevents race conditions where multiple requests could reserve the same username
    try {
      await runTransaction(db, async (transaction) => {
        ...
        // Check existing VIP reservations (unclaimed)
        ...
        if (!vipSnapshot.empty) {
          throw new Error('USERNAME_ALREADY_RESERVED');
        }

        // Create reservation atomically
        const reservationRef = doc(db, 'vip_reservations', reservationId);
        transaction.set(reservationRef, reservation);

        // Log the action
        const auditRef = doc(db, 'admin_audit_log', `${reservationId}_created`);
      ...
      if (error.message === 'USERNAME_ALREADY_RESERVED') {
        const errorResponse = createErrorResponse(
          ErrorType.VALIDATION,
