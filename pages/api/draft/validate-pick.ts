...
import { validatePickRequestSchema } from '../../../lib/validation/draft';
...
export default async function handler(
  ...
  return withErrorHandling(req, res, async (req, res, logger) => {
    ...
    if (!authResult.authenticated) {
      serverLogger.warn('Draft validate-pick auth rejected', null, {
    ...
    const { roomId, userId, playerId, pickNumber } = validatePickRequestSchema.parse(req.body);

    // ── IDOR Prevention ─────────────────────────────────────────────
    if (authResult.uid !== userId) {
      serverLogger.warn('Draft validate-pick IDOR attempt', null, {
      ...
      return res.status(403).json({
        valid: false,
        errorCode: 'FORBIDDEN',
        errorMessage: 'Cannot validate picks for another user',
      });
    }
    ...
    logger.info('Pick validated successfully', {
      ...
      logger.error('Unexpected error validating pick', error instanceof Error ? error : new Error(String(error)));
