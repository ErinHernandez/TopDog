 ...
 *
 * Configuration:
 * - 200 requests per minute per IP for general rate limiting (Stripe sends more webhooks)
...
const generalWebhookLimiter = new RateLimiter({
  endpoint: 'stripe_webhook_general',
  maxRequests: 200, // Stripe sends high volume
...
const failedVerificationLimiter = new RateLimiter({
  endpoint: 'stripe_webhook_failed_verification',
  maxRequests: 10,
  windowMs: 60 * 1000, // 1 minute
  failClosed: true,
  circuitBreakerThreshold: 5,
...
export default async function handler(
  ...
  return withErrorHandling(req, res, async (req, res, logger) => {
    ...
    if (!generalRateLimitResult.allowed) {
      logger.warn('Stripe webhook rate limited - too many requests', {
        component: 'stripe',
        operation: 'webhook',
        ip: clientIp,
      ...
      // Note: Stripe will retry, but this prevents resource exhaustion attacks
    ...
    try {
      ...
      if (!rateLimitResult.allowed) {
        logger.warn('Stripe webhook brute force detected - rate limited', {
          component: 'stripe',
          operation: 'webhook',
          ip: clientIp,
