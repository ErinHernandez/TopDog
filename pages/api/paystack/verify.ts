...
import { paystackVerifySchema } from '../../../lib/validation/payment';
...
export default async function handler(
  ...
  return withErrorHandling(req, res, async (req, res, logger) => {
    ...
    // Get reference from query (GET) or validated body (POST)
    let reference: string;

    if (req.method === 'GET') {
      validateQueryParams(req, ['reference'], logger);
      const ref = req.query.reference;
      reference = typeof ref === 'string' ? ref : Array.isArray(ref) ? ref[0] ?? '' : '';
      if (!reference.trim()) {
      ...
      const body = validateRequestBody(req, paystackVerifySchema, logger);
