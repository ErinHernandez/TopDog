...
// FIX B: Webhook authentication configuration
...
export default async function handler(
  ...
  await withErrorHandling(req, res, async (req, res, logger) => {
    ...
    // FIX B: Validate XENDIT_WEBHOOK_TOKEN and XENDIT_WEBHOOK_SECRET exist BEFORE processing
    if (!XENDIT_WEBHOOK_TOKEN) {
      logger.error('Webhook token not configured', new Error('XENDIT_WEBHOOK_TOKEN missing'), {
        component: 'xendit',
        operation: 'webhook_config',
      });
      return res.status(500).json({
        received: false,
        error: 'Webhook configuration error - XENDIT_WEBHOOK_TOKEN not set'
      });
    }

    if (!XENDIT_WEBHOOK_SECRET) {
      logger.error('Webhook secret not configured', new Error('XENDIT_WEBHOOK_SECRET missing'), {
        component: 'xendit',
        operation: 'webhook_config',
      });
      return res.status(500).json({
        received: false,
        error: 'Webhook configuration error - XENDIT_WEBHOOK_SECRET not set'
      });
    }
    ...
    try {
      ...
      // SECURITY: Verify HMAC signature for payload integrity
      const webhookSignature = req.headers['x-xendit-signature'] as string;
      if (!verifyWebhookSignature(bodyString, webhookSignature)) {
