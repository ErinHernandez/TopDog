...
import { quickPickRequestSchema } from '../../../../lib/validation/draft';
...
export default async function handler(
  ...
  return withErrorHandling(req, res, async (req, res, logger) => {
    ...
    const { userId, playerId, location, deviceId } = quickPickRequestSchema.parse(req.body);
    ...
    try {
      ...
      return res.status((statusMap[code as keyof typeof statusMap]) || 500).json({
        success: false,
        error: {
          code,
          message,
