...
function removeAllClayProjections() {
  ...
  try {
    ...
    const startIndex = playerPoolContent.indexOf('export const PLAYER_POOL = [');
    const endIndex = playerPoolContent.lastIndexOf('];');
    
    if (startIndex === -1 || endIndex === -1) {
      throw new Error('Could not find PLAYER_POOL array in playerPool.js');
    }
    
    const arrayContent = playerPoolContent.substring(startIndex + 26, endIndex + 1);
    const playerPool = JSON.parse(arrayContent);
