...
async function extractClayProjections2025(): Promise<ClayProjection[]> {
  ...
  try {
    ...
    projections.slice(0, 10).forEach((player: ClayProjection, index: number) => {
      console.log(`${index + 1}. ${player.name ?? 'Unknown'} (${player.position ?? 'Unknown'}, ${player.team ?? 'Unknown'})`);
  ...
  try {
    ...
    if (position === 'QB') {
      ...
      if (parts.length >= 10) {
        ...
        player.team = parts[2];
        player.games = parseInt(parts[3] ?? '0') || 0;
        player.passing = {
          attempts: parseInt(parts[4] ?? '0') || 0,
          yards: parseInt(parts[5] ?? '0') || 0,
          tds: parseInt(parts[6] ?? '0') || 0,
          ints: parseInt(parts[7] ?? '0') || 0
        };
        player.rushing = {
          attempts: parseInt(parts[8] ?? '0') || 0,
          yards: parseInt(parts[9] ?? '0') || 0,
          tds: parseInt(parts[10] ?? '0') || 0
        };
        player.fantasy = {
          ppr: parseFloat(parts[11] ?? '0') || 0,
          halfPpr: parseFloat(parts[11] ?? '0') || 0
        };
      }
    } else if (position === 'RB') {
      ...
      if (parts.length >= 11) {
        ...
        player.receiving = {
          targets: parseInt(parts[7] ?? '0') || 0,
          receptions: parseInt(parts[8] ?? '0') || 0,
          yards: parseInt(parts[9] ?? '0') || 0,
          tds: parseInt(parts[10] ?? '0') || 0
        };
        const pprVal: number = parseFloat(parts[11] ?? '0') || 0;
        player.fantasy = {
          ppr: pprVal,
      ...
      if (parts.length >= 11) {
        ...
        player.games = parseInt(parts[3] ?? '0') || 0;
        player.receiving = {
          targets: parseInt(parts[4] ?? '0') || 0,
          receptions: parseInt(parts[5] ?? '0') || 0,
          yards: parseInt(parts[6] ?? '0') || 0,
          tds: parseInt(parts[7] ?? '0') || 0
        };
        player.rushing = {
          attempts: parseInt(parts[8] ?? '0') || 0,
          yards: parseInt(parts[9] ?? '0') || 0,
